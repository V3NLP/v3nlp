...
import gov.va.research.inlp.LabelValue;
...
public class ExpressionServiceImpl {
	...
	public Collection<Expression> getAllExpressions() {
		Collection<Expression> expressions = this.jdbcTemplate.query(
	...
	public Collection<Expression> findExpressions(String haystack, String needle) {
		if (GenericValidator.isBlankOrNull(haystack) || GenericValidator.isBlankOrNull(needle)) {
			throw new RuntimeException("Haystack and needle must both have a value. ");
		}
		
		Collection<Expression> expressions = this.jdbcTemplate.query(
			    "select id, title, expression, description, created_by, created_date, matches, non_matches, citation from expressions where " + haystack + " like '%" + needle.replaceAll("'", "''") + "%'",
			    new RowMapper<Expression>() {
			        public Expression mapRow(ResultSet rs, int rowNum) throws SQLException {
			            Expression exp = new Expression();
			            exp.setId(rs.getInt("id"));
